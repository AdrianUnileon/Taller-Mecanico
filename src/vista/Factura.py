...
class Facturas(QMainWindow):
    ...
    def cargar_facturas(self):
        ...
        for orden in datos:
            precio_total = round(Decimal(float(orden["CostoManoObra"] * Decimal("1.36"), 2)))
            descripcion = (
    ...
    def generar_factura(self):
        ...
        costo_mano_obra = Decimal(float(orden_seleccionada["CostoManoObra"]))
...
class Facturas(QMainWindow):
    ...
    def cargar_facturas(self):
        ...
        for orden in datos:
            try:
                # Conversión segura: primero a string y luego a Decimal
                costo_mano_obra = Decimal(str(orden["CostoManoObra"]))
                precio_total = round(costo_mano_obra * Decimal("1.36"), 2)
                nombre_cliente = orden.get('NombreCliente', 'Desconocido')
                descripcion = (
                    f"ID: {orden['IDOrden']} - {orden['Descripcion']} - "
                    f"Estado: {orden['Estado']} - Precio: {precio_total:.2f} € - "
                    f"Cliente: {nombre_cliente} - Vehículo: {orden['Vehiculo']}"
                )
                self.comboFacturas.addItem(descripcion, orden["IDOrden"])
            except (TypeError, ValueError) as e:
                print(f"Error al procesar orden {orden.get('IDOrden', 'N/A')}: {str(e)}")
                continue
    ...
    def generar_factura(self):
        ...
        try:
            costo_mano_obra = Decimal(str(orden_seleccionada["CostoManoObra"]))
            total_factura = round(costo_mano_obra * Decimal("1.36"), 2)
        except (TypeError, ValueError) as e:
